...
class DisableHostCheckMiddleware:
    ...
    def __call__(self, request):
        ...
        def patched_get_host():
            try:
                ...
                logger.warning(f"DisallowedHost bypassed for: {request.META.get('HTTP_HOST', 'unknown')}")
                return 'localhost'
    ...
    def process_exception(self, request, exception):
        if isinstance(exception, DisallowedHost):
            logger.warning(f"DisallowedHost exception caught for: {request.META.get('HTTP_HOST', 'unknown')}")
